...
@WebServlet("/UpdateMedicineList")

public class UpdateMedicineList extends HttpServlet
{
	String msg="";
	...
	{		
		...
		int soldMedicineQuantity=0;
		int availableMedicineQuantity=0;
		...
		soldMedicineQuantity = Integer.parseInt(request.getParameter("soldMedicineQuantity"));
		availableMedicineQuantity = Integer.parseInt(request.getParameter("availableMedicineQuantity"));
		...
		if(medicineType.equals("Allergies"))
		{
			...
			{
				msg = "Medicine is not available in the medicine store...!";
			}
		}
		...
		/*else if(medicineType.equals("MedicineAccessory"))
		{
			allAccessory = MySqlDataStoreUtilities.getAllergies();
			if(allAccessory.containsKey(medicineId))
			{
				msg = "Medicine Accessory is not available in the medicine store...!";
			}
		}*/
		if (msg.equals("good"))
		{  
			try
			{
				msg = MySqlDataStoreUtilities.updatemedicines(medicineType,medicineId,medicineName,medicinePrice,medicineImage,medicineManufacturer,medicineManufactureDate,medicineBestBeforeDate,medicineDescription,medicineDiscount,originalMedicineQuantity,soldMedicineQuantity,availableMedicineQuantity,manufacturerRebate);
			}
			catch(Exception e)
			{ 
				msg = "Medicine cannot be updated...!";
			}
			//msg = "Medicine has been updated successfully...!";
		}	
		displayUpdateMedicine(request, response, true);	
	}

	protected void displayUpdateMedicine(HttpServletRequest request, HttpServletResponse response, boolean error) throws ServletException, IOException
	{
		response.setContentType("text/html");
		PrintWriter pw = response.getWriter();
		UtilitiesMedicineManager utility = new UtilitiesMedicineManager(request,pw);
		...
		pw.print("<h2><a style='margin-left:15px;font-weight: normal;font-family:Century Schoolbook;color:white;font-size:34px;text-decoration: none;'>Update Medicine</a></h2></div>");
		pw.print("<br>");
		pw.print("<div id='entry'><br>");
		if (error)
			pw.print("<h3 style='margin-left:400px;font-family:Century Schoolbook;color:#e50000;'>"+msg+"</h3>");
		pw.print("<form method='post' action='UpdateMedicineList'>"
				+ "<table style='width:100%'><tr><td style='width:650px'>"
				+"<br><h3 style='margin-left:15px;color:black;'>Medicine Category <font color='red'>*</font></h3></td><td><select style='margin-left:-260px;margin-top:20px;width:250px;height:35px;' name='medicineType' class='input'><option value='Allergies' selected>Allergies</option><option value='Antibiotics'>Antibiotics</option><option value='AntiViral'>Anti Viral</option><option value='HeartDisease'>Heart Disease</option><option value='Kidney Disease'>Kidney Disease</option><option value='Skincare'>Skincare</option><option value='MedicineAccessory'>Medicine Accessory</option></select>"
				+ "</td></tr><tr><td>"
				...
				+ "</td></tr><tr><td>"				
				+ "<h3 style='margin-left:15px;color:black;'>Sold Medicine Quantity <font color='red'>*</font></h3></td><td><input size='75' style='margin-left:-260px;margin-top:20px;width:250px;height:35px;' type='number' step='any' placeholder='please enter numeric data' name='soldMedicineQuantity' value='' class='input' required></input>"
				+ "</td></tr><tr><td>"
				+ "<h3 style='margin-left:15px;color:black;'>Available Medicine Quantity <font color='red'>*</font></h3></td><td><input size='75' style='margin-left:-260px;margin-top:20px;width:250px;height:35px;' type='number' step='any' placeholder='please enter numeric data' name='availableMedicineQuantity' value='' class='input' required></input>"
				+ "</td></tr><tr><td>"				
				...
				+ "</td></tr></table>"
				+ "<input type='submit' class='btn_medicine' name='button' value='Update Medicine Details'></input>"
				+ "</form>" + "<br><br></div>");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter pw = response.getWriter();
		UtilitiesMedicineManager utility = new UtilitiesMedicineManager(request,pw);		
		displayUpdateMedicine(request, response, false);
	}
}
